...
Incorrect input data processing is important as well as notifying client. [HTTP status codes](http://en.wikipedia.org/wiki/List_of_HTTP_status_codes) is general simple concept to notify client about state of request processing results. Error message in JSON response complements response and clarifies error for end users.

Here is list of HTTP status codes that we will use:

* 200, 201 - Success OK and Created (201 for POST request)
* 404      - Not Found
* 400, 422 - Bad Request, Unprocessable Entity
* 401, 403 - Unauthorized, Forbidden
* 500      - Internal Server Error

400 and 422 is similar, we will use 400 to notify about invalid params in general (invalid type or value out of range) and 422 for validation errors.  
...
With current service implementation if unexpected error occurs service responds with 500 error with no response body. Whereas we develop JSON service we should notify client with error message in JSON format (in `application.rb`).
...
If client provides Zip code IP that does not exist in database activerecord `find` method raises `ActiveRecord::RecordNotFound` error and service responds with 500 HTTP status code. This is not good. We should respond with 404 status code and appropriate error message.
...
If we will try to save model with invalid data (e.g. wrong format zip) activerecord will raise ActiveRecord::RecordInvalid error. In this case we should respond with 422 HTTP status code and with specific validation error messages.
...
We can test this all this catching in acceptance tests. Here is updated `spec/acceptance/zip_codes_spec.rb`
...
Sometimes we need ensure correctness of parameter types provided by client. We can prevent interaction with database if provided incorrect parameters and provide error message to client immediately. We will use gem [sinatra-param](https://github.com/mattt/sinatra-param) for parameter validation and type coercion in sinatra.

Please add line `gem 'sinatra-param'` in `Gemfile`
...
And rub `bundle install`

If client will provide parameter zip_code of wrong type (not Hash) servicew will respond with 500 error (something like `NoMethodError: undefined method `stringify_keys' for "STRING":String` if provide String instead Hash). We can achieve this with next declaration in `app/controllers/zip_codes_controller.rb`.
...
We can go further and validate parameter zip in get request with regular expression. This prevents call to the database.
And service will respond with 400 HTTP status code instead 404, also error message is more appropriate ("Record not found" provides that record is possible has been removed but message "Parameter must match format" does not).
...
I have limitation for integer type in my database. If client provides Zip code ID more than 2147483647 (which is binary 111111111111111111111111111111) activerecord raises postgresql error and service responds with 500 error. It is possible to prevent this with next declarations.
...
Here is updated tests (file `spec/acceptance/zip_codes_spec.rb`)
...
We have envisaged error catching end responding with appropriate HTTP status codes and errr messages in JSON format (we have used sinatra's error handling ability). We used gem [sinatra-param](https://github.com/mattt/sinatra-param) for parameter validation and type coercion in sinatra.
